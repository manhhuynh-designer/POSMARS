...
/**
 * Virtual Watch & Ring Try-on using WebAR.rocks Hand
 * Tracks hands for accurate wrist and finger placement
 */
export default function WebARRocksHand({
    config,
    onComplete,
    onCapture
}: WebARRocksHandProps) {
    ...
    useEffect(() => {
        const init = async () => {
            try {
                ...
                console.error('Failed to initialize WebAR.rocks Hand:', e)
                setError('Không thể khởi động Hand Tracking')
                setLoading(false)
            }
        }
    ...
    const initARScene = () => {
        ...
        console.log('Hand preference:', config.hand_preference)
        console.log('Accessory models:', config.accessory_models)
    ...
    const handleStartRecord = () => {
        ...
        if (video) {
            startRecording(video, arCanvas, true) // Mirror for hand tracking
        }
    }
    ...
    return (
        <div className="fixed inset-0 bg-black">
            ...
            <AROverlay
                ...
                instructions={config.instructions || 'Show your hand to try on accessories'}
                showScanHint={!handDetected && arReady}
                onClose={onComplete}
            />
            ...
            {loading && (
                <div className="absolute inset-0 flex items-center justify-center bg-black/80 z-20">
                    <div className="text-center text-white">
                        ...
                        <p>Đang khởi động Hand Tracking...</p>
                    </div>
                </div>
            )}
